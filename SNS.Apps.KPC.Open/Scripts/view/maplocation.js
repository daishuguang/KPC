...
function initialize(flag) {
    var city_location = globals_currentcity + globals;
	map.reset();
    // map.centerAndZoom
    var mapoption = {
    ...
    //map = new BMap.Map("mapdiv", mapoption);
    ...
    map.clearOverlays();
    if (coordinate) {
        point = new BMap.Point(coordinate.longitude, coordinate.latitude);
        ...
        if (city_location !== null && city_location !== "" && city_location !== undefined) {
            ...
            point = new BMap.Point(locationPoint.longitude, locationPoint.latitude);
            if (flag)
            ...
            myGeo.getLocation(point, function (result) {
                if (result) {
                    ...
                    globals_currentcity = add.city;
                    //globals = add.district + add.street + add.streetNumber;
                    globals = add.street + add.streetNumber;
            ...
            if (latestAddress == null) {
                ...
                point = new BMap.Point(latestAddress.lng, latestAddress.lat);
                map.centerAndZoom(point, 14);
            }
        }
